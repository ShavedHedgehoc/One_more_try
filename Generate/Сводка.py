...
if __name__ == "__main__":
    xl_file = {"Копия Отчет.xlsx", "Копия Отчет2.xlsx"}
    df_summary = None
    df_batch = None
    for one_file in xl_file:
        ...
        for index, row in df2.iterrows():
            ...
            if pd.isna(row[1]):
                ...
                df_batch = pd.concat([df_batch, batch_row], ignore_index=True)
            elif row[0] == "код":
                pass
            else:
                summary_row = pd.DataFrame(
                    [[cur_date, row[0], row[3], row[4], row[8]]],
    ...
    for index, row in df.iterrows():
        if (
            ...
            df_res = pd.concat([df_res, reg_row], ignore_index=True)
    df_res.to_excel("Регламент2.xlsx", index=False, header=True)
